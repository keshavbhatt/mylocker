...
SmartVerticalFlowLayout::SmartVerticalFlowLayout(QWidget *parent)
    : QLayout(parent), m_hSpace(-1), m_vSpace(-1), m_maxRowCount(-1),
      m_isLayoutModified(false) {
  setContentsMargins(-1, -1, -1, -1);
}

SmartVerticalFlowLayout::~SmartVerticalFlowLayout() { qDeleteAll(m_items); }
...
int SmartVerticalFlowLayout::maxRowCount() const { return m_maxRowCount; }
...
int SmartVerticalFlowLayout::count() const { return m_items.size(); }

void SmartVerticalFlowLayout::addItem(QLayoutItem *item) {
  // m_isLayoutModified = true;// is useless here as long as QLayout keeps
  // calling setGeometry
  m_items.append(item);
}
...
QSize SmartVerticalFlowLayout::minimumSize() const { return sizeHint(); }
